...
#' @export
Query.miRNetDB <- function(db.path, q.vec, table.nm, col.nm, db.nm = "mirtarbase", gene.db = ""){
  ...
  }else if(grepl("mir2gene", db.path) && nzchar(gene.db)){
    # gene-input direction: restrict to one validated source when requested
    statement <- paste("SELECT * FROM ", table.nm, " WHERE ", col.nm," IN (", query, ")"," AND ", gene.db," == 1 ", sep="");
  }else{
...
# Helper to get miRNet object from global env
.get.mirnet.obj <- function(mSetObj = NA) {
  if (is.na(mSetObj) || is.null(mSetObj)) {
    if (exists("mSet", envir = .GlobalEnv)) {
      return(get("mSet", envir = .GlobalEnv))
    }
  }
  return(mSetObj)
}

# Helper to set miRNet object in global env
.set.mirnet.obj <- function(mSetObj) {
  assign("mSet", mSetObj, envir = .GlobalEnv)
  return(mSetObj)
}
...
rsclient_isolated_exec <- function(func_body, input_data, packages = character(0),
  ...
  result <- run_func_via_microservice(
    func = function(input_path, output_path, func_body, pkgs) {
      tryCatch({
        ...
        input_data <- ov_qs_read(input_path)
        res <- func_body(input_data)
        ov_qs_save(res, output_path, preset = "fast")
        Sys.sleep(0.02)
        list(success = TRUE)
      }, error = function(e) {
  ...
  # The subprocess returns invisibly: the closure's result travels via the output file, not the
  # return value, so gate success on the output file existing (the closure writes it only on a
  # successful run). Checking the return value made every call look failed, leaving the layout
  # empty so pos.xy came back malformed ("incorrect number of dimensions").
