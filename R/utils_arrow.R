...
#'
#' For network analysis results. Returns verified path.
...
#' Export network node table to Arrow format (Safe-Handshake)
#'
#' For node attribute tables in network analysis. Returns verified path.
#'
#' @param node_table Node table data frame
#' @param filename Output filename (without .arrow extension)
#' @return The verified absolute path to the Arrow file, or NULL on failure
#' @export
ExportNodeTableArrow <- function(node_table, filename) {
    tryCatch({
        df <- as.data.frame(node_table, stringsAsFactors = FALSE)
...
#' Export enrichment results to Arrow format (Safe-Handshake)
#'
#' For functional enrichment analysis results. Returns verified path.
#'
#' @param enrich_table Enrichment result table
#' @param filename Output filename (without .arrow extension)
#' @return The verified absolute path to the Arrow file, or NULL on failure
#' @export
ExportEnrichmentArrow <- function(enrich_table, filename) {
    tryCatch({
        df <- as.data.frame(enrich_table, stringsAsFactors = FALSE)
        ...
        rn <- rownames(enrich_table)
        if (!is.null(rn) && !all(rn == as.character(1:nrow(df)))) {
            df <- cbind(row_names_id = as.character(rn), df)
        }
        ...
        warning(paste("ExportEnrichmentArrow failed:", e$message))
        return(NULL)
    })
}
